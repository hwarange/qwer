...
class MNISTModule(L.LightningModule):
    ...
    def training_step(self, batch):
        ...
        self.train_loss = F.mse_loss(x_reconstructed, X)   # 예측값과 실제 값 간의 손실 계산 (MSE 손실 함수 사용)
    ...
    def on_train_epoch_end(self, *args, **kwargs):
        ...
        self.log_dict(
            {'loss/train_loss': self.train_loss},  # 학습 손실을 로그에 기록
            on_epoch=True,
    ...
    def test_step(self, batch, batch_idx):
        # 테스트 단계에서 호출되는 메서드
        X = batch.get('X')  # 입력 데이터를 가져옴X = batch.get('X')  # 입력 데이터를 가져옴
        y = batch.get('y')
